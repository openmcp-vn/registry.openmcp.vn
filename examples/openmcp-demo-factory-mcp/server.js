...
const info = { name: "OpenMCP Factory Demo MCP", version: "0.1.0" };
...
readline.createInterface({ input: process.stdin, terminal: false }).on("line", line => {
  ...
  if (request.method === "initialize") out(request.id, { protocolVersion:"2024-11-05", capabilities:{tools:{}}, serverInfo:info });
  else if (request.method === "tools/list") out(request.id, { tools });
  else if (request.id !== undefined) out(request.id, { content:[{type:"text", text:"Generated MCP response"}] });
});
